  ...
  return (
        <div className="grid grid-cols-2 gap-4 bg-white rounded-[10px] p-6 md:flex md:items-center">
            <Link to={`/suggestions/${id}`} className="group col-span-2 flex flex-col items-start space-y-2 md:flex-1">
                <h3 className="text-sm font-bold tracking-tight text-veryDarkBlue group-hover:text-mainBlue">{title}</h3>
            ...
            <button
            onClick={() => upvote()}
            className={`${isUpvoted ? "bg-mainBlue text-white" : "bg-grey text-veryDarkBlue"} group flex items-center place-self-start space-x-2 px-4 py-2 font-bold text-body-sm  rounded-[10px] md:-order-1 md:flex-col md:items-center md:space-x-0 md:space-y-2 md:mr-10 hover:bg-grey-hover hover:text-veryDarkBlue`}
            >
                {/* <img src="/img/shared/icon-arrow-up.svg" className="" /> */}
                <svg className={`${isUpvoted ? "stroke-grey" : "stroke-mainBlue"} group-hover:stroke-mainBlue`} width="10" height="7" xmlns="http://www.w3.org/2000/svg"><path d="M1 6l4-4 4 4" strokeWidth="2" fill="none" fillRule="evenodd"/></svg>
