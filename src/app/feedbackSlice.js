...
export const feedbackSlice = createSlice({
  ...
  reducers: {
    ...
    getNumberOfFeedback: (state, action) => {
      state.feedback.reduce((acc, curr) => {
        if (action.payload === curr.status) {
          acc++;
        }
        return acc;
      }, 0);
...
export const { changeCategory, getNumberOfFeedback } = feedbackSlice.actions;
